if (Meteor.isClient){
	...
	Template.newPost.events ({
		"click #back_post_button" : function() {
		...
		"submit form#edit": function(event) {
			...
			var id = parts.pop();
			Posts.update(id, {
				$set: {title: title, content: content, createdAt : date}
			});
			Router.go('admin/blog');
		},
		"keypress textarea[name='content']" : function(event) {
			if (event.which === 13) {
				$('form').submit();
			}
		}
    ...
    id:'post-content', // (required) ID of the textarea

    // width:80, // (optional) width of the editor
    ...
    controls:['bold', 'italic', 'underline', 'strikethrough', '|', 'subscript', 'superscript', '|', 'orderedlist', 'unorderedlist', '|' ,'outdent' ,'indent', '|', 'leftalign', 'centeralign', 'rightalign', 'blockjustify', '|', 'unformat', '|', 'undo', 'redo', 'n', 'style', '|', 'image', 'hr', 'link', 'unlink', '|', 'print'], // (required) options you want available, a '|' represents a divider and an 'n' represents a new row
